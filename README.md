...
The function 'SplitMerge' split the source image to 3 channels, find each image's edge through Canny, then bitwise AND and merge 3 new edges, which may get good edges.  
...
(1) CompressChannel: compress the value of each channel from 256 kinds to 16 kinds, this improves the contrast of the image, but may lead to some obvious edges in the continuous color block.  
